...
def main(unused_argv):
    ...
    classifier = tf.estimator.Estimator(
        # model_fn=model.build_base_model, #build_base_model
        model_fn=model.PLE, #ESMM、MMoE
        ...
        params = {
            ...
            "expert_net_num": FLAGS.expert_net_num,
            "units": FLAGS.units,
            "ple_levels": FLAGS.ple_levels
        }
    )
...
max_steps=FLAGS.train_steps)
        eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_data.eval_input_fn(FLAGS.test_record_dir, FLAGS.eval_batch_size),
                                          start_delay_secs=60,
                                          throttle_secs = 30,
                                          steps=200)
